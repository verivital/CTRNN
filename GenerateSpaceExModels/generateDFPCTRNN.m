...
%Generate Hybrid Automaton of Damped Forced Pendulum CTRNN
%-------------------
clear
clc;
close all
...
initConditionStr=strcat(var{1}," ",'==',num2str(x0(1)));
for j=2:length(var)
    initConditionStr=strcat(initConditionStr,' & '," ",var{j},'==',num2str(x0(j)));
end
flowstr=ODEs1{1};
for i=2:(length(ODEs1))
    flowstr=strcat(flowstr," ",'&'," ",ODEs1{i});
end
